...
export async function* parseShapefileInBatches(
  ...
  const shapeIterable: any = await context.parseInBatches(
    asyncIterator,
    SHPLoader,
    options
  );
  ...
  // @ts-ignore context must be defined
  const dbfResponse = await context.fetch(
    replaceExtension(context?.url || "", "dbf")
  );
  if (dbfResponse.ok) {
    // @ts-ignore context must be defined
...
export async function parseShapefile(
  ...
  // parse geometries
  // @ts-ignore context must be defined
  const { header, geometries } = await context.parse(
    arrayBuffer,
    SHPLoader,
    options
  ); // {shp: shx}
  ...
  // @ts-ignore context must be defined
  // ここを修正した
  const dbfResponse = await context.fetch(options.files);
